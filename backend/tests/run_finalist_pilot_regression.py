"""v1.5.0: gerçek kullanıcı verisi toplamayan kontrollü demo senaryosu."""
...
class PilotApiTests(unittest.TestCase):
    ...
    def test_12_real_user_mode_is_disabled(self):
        response = self.client.post('/api/pilot/sessions', json={'consent': True, 'practice': False})
        self.assertEqual(response.status_code, 400)
        self.assertIn('gerçek kullanıcı verisi', response.json()['detail'].casefold())

    def test_13_demo_pairs_never_enter_user_metrics(self):
        self.complete(practice=True)
        overview = self.client.get('/api/pilot').json()
        self.assertEqual(overview['active_session_count'], 0)
        self.assertEqual(overview['completed_session_count'], 0)
        self.assertEqual(overview['raw']['completed_task_count'], 0)

    def test_14_demo_overview_never_creates_conclusion(self):
        self.complete(practice=True)
        overview = self.client.get('/api/pilot').json()
        self.assertFalse(overview['minimum_sample_reached'])
        self.assertIn('gerçek kullanıcı verisi', overview['conclusion'].casefold())

    def test_15_repeated_demo_runs_stay_out_of_metrics(self):
        for _ in range(MINIMUM_SAMPLE_SIZE):
            self.complete(practice=True)
        overview = self.client.get('/api/pilot').json()
        self.assertFalse(overview['minimum_sample_reached'])
        self.assertEqual(overview['completed_session_count'], 0)
        self.assertEqual(overview['practice_session_count'], MINIMUM_SAMPLE_SIZE)

    def test_16_csv_contains_no_user_rows(self):
        self.complete(practice=True)
