"""v1.5.0: mobil, erişilebilir, sunum ve kontrollü demo arayüz sözleşmesi."""
...
class FinalistUIContractTests(unittest.TestCase):
    ...
    def test_11_privacy_mode_masks_comment_authors(self):
        ...
        self.assertIn('privacyMode={privacyMode}', PAGE)
        self.assertIn('visibleAuthor(comment.author, comment.id, privacyMode)', PAGE)
    ...
    def test_15_presentation_links_to_live_product_evidence(self):
        ...
        self.assertIn('Kontrollü Senaryoyu Aç', PAGE)

    def test_16_pilot_is_primary_navigation(self):
        self.assertIn("'Kontrollü Senaryo'", PAGE)
        self.assertIn('<PilotWorkspace', PAGE)
        self.assertIn('<PilotPanel', PAGE)

    def test_17_pilot_api_is_fully_connected(self):
        for path in ('/api/pilot`', '/api/pilot/sessions`', '/api/pilot/sessions/${sessionId}/phases'):
            self.assertIn(path, API)
        self.assertNotIn('downloadPilotResults', API)

    def test_18_pilot_requires_consent(self):
        pilot_source = (ROOT / 'backend' / 'app' / 'pilot.py').read_text(encoding='utf-8')
        self.assertIn('Kontrollü denemeyi başlatmak için onay', MAIN + pilot_source)
        self.assertIn('startPilotSession(true, true)', PAGE)

    def test_19_practice_mode_is_explicit(self):
        self.assertIn('Sabit demo senaryosu', PAGE)
    ...
    def test_21_demo_does_not_expose_user_evidence_export(self):
        self.assertNotIn('downloadPilotResults()', PAGE)
        self.assertIn('Gerçek sonuç üretilmiyor', PAGE)
    ...
    def test_30_presentation_explains_controller_flow_and_demo_readiness(self):
        for term in ('Jüri akışı', 'ilk basış demoyu hazırlar', 'Canlı demo hazırlanıyor…', 'Demo verisini hazırla'):
            self.assertIn(term, PAGE)
        self.assertIn('demoReady', PAGE)
        self.assertIn('demoLoading', PAGE)

    def test_33_presentation_evidence_buttons_prepare_and_restore_cached_demo(self):
        self.assertIn("if (actionLoading) return;", PAGE)
        self.assertIn('setPresentationDemoPrepared(true)', PAGE)
        self.assertIn('setPost(demo);', PAGE)
        self.assertIn('setAnalysis(result);', PAGE)
        self.assertIn("disabled={actionLoading}", PAGE)

    def test_34_failed_form_actions_keep_user_input_and_retry_state(self):
        self.assertIn('Promise<boolean>', PAGE)
        self.assertIn('if (!created) return;', PAGE)
        self.assertIn('if (saved) setEditing(false);', PAGE)
        self.assertIn("disabled={rewriteLoading || !draft.trim()}", PAGE)
        self.assertIn('maxLength={2000}', PAGE)
        self.assertIn("coachStatus?.mode === 'coach-fallback'", PAGE)
        self.assertIn('coachStatus?.installed', PAGE)
