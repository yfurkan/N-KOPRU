"""N-KÖPRÜ kontrollü demo senaryosu.

Finalist teslimi gerçek kullanıcı verisi toplamaz ve gerçek etki sonucu
üretmez. Bu modül yalnızca iki sabit senaryoyu, ham yorumlar ile N-KÖPRÜ
çıktısını aynı akışta göstermek için yerel deneme kayıtlarıyla çalıştırır.
Eski istemcilerle uyumluluk için ``/api/pilot`` adı korunur; gerçek oturum
ve gerçek metrik yolu bilinçli olarak kapalıdır.
"""
...
PROTOCOL_VERSION = 'v1.5.0-controlled-demo'
...
def start_session(*, consent: bool, practice: bool) -> dict[str, Any]:
    if not consent:
        raise ValueError('Kontrollü denemeyi başlatmak için onay gereklidir.')
    if not practice:
        raise ValueError(
            'Bu finalist teslimi gerçek kullanıcı verisi toplamaz; yalnızca sabit demo denemesi kullanılabilir.'
        )
...
def submit_phase(
    ...
    *,
    ...
    with transaction(immediate=True) as conn:
        ...
        if session is None:
            raise KeyError('Kontrollü demo oturumu bulunamadı.')
        if not bool(session['practice']):
            raise ValueError(
                'Bu finalist teslimi gerçek kullanıcı verisi toplamaz; gerçek oturum sonuçları işlenmez.'
            )
        ...
        if existing is not None:
            ...
            if phase_index != completed_count:
                raise ValueError('Kontrollü demo adımları sırayla tamamlanmalıdır.')
...
def get_overview() -> dict[str, Any]:
    ...
    # Finalist tesliminde kullanıcı metriği hesaplanmaz. Deneme kayıtları,
    # yalnızca arayüzün iki aşamalı akışını tekrar çalıştırabilmek içindir.
    raw = _variant_metrics([], 'raw')
    nkopru = _variant_metrics([], 'nkopru')
    minimum_reached = False
    conclusion = (
        'Bu finalist teslimi gerçek kullanıcı verisi veya etki sonucu toplamaz. '
        'Ekrandaki akış yalnızca sabit senaryolu yerel denemedir; skorlar gerçek '
        'kullanıcı performansı olarak yorumlanamaz.'
    )

    return {
        'protocol_version': PROTOCOL_VERSION,
        'recommended_participants': 'Gerçek katılımcı yok; yalnızca sabit demo denemesi',
        ...
        'raw': raw,
        'nkopru': nkopru,
        'time_gain_percent': None,
        'accuracy_gain_points': None,
        'clarity_gain': None,
        'conclusion': conclusion,
        'integrity_note': (
            'İsim, e-posta, telefon veya serbest metin katılımcı verisi toplanmaz. '
            'Deneme kayıtları yalnız yerel SQLite akışını doğrulamak içindir; '
            'gerçek oturum ve gerçek metrik yolu kapalıdır.'
        ),
    }
