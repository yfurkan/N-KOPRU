# N-KÖPRÜ — Sürüm Geçmişi

## v1.5.0 — Finalist kaynak teslimi

- Jüri için 4:30 sayaçlı **Sunum Modu**, canlı hazırlık denetimi ve beş
  adımlı anlatı akışı eklendi.
- Gerçek kullanıcı toplamayan **Kontrollü Senaryo** akışı; iki sabit tartışma,
  iki görünüm ve tekrar çalıştırılabilir yerel görevlerle sınırlandı. Gerçek
  metrik ve kullanıcı CSV’si yolu kapatıldı.
- Ortak zemin ve Köprü dili tartışma konusunun gerçek karar ölçütlerine bağlandı.
- Mobil menü/analiz çekmecesi, skip-link, canlı bölge, klavye sekme gezinmesi,
...
- GitHub teslim hedefi main dalıdır; Actions kalite kapısı push sonrasında
  yeniden çalışır.
- Gerçek kullanıcı etkisi, canlı platform sonucu veya saha başarısı iddia
  edilmez.

## v1.4.2 — Konuya Duyarlı Analiz ve Ayrılmış İç Kontrol

- Görüş Haritası, Tartışmayı Anla ve Köprü Oluştur aynı konu bağlamıyla
  uzaktan çalışma, telefon, park, kantin, geri dönüşüm, kütüphane, dijital
  oyun, bisiklet ve ulaşım tartışmalarına anlamlı görüş başlıkları üretir.
- Kanonik görüş kimlikleri, SQLite kayıtları, bildirim parmak izleri ve sabit
  akademik yapay zekâ demosu değiştirilmez.
- Önceki dört konulu 80 kalibrasyon örneğiyle çakışmayan beş yeni konu ve
  80 yeni ifade için üçüncü, ayrı proje içi doğrulama ekranı eklenmiştir.
- 0 ortak cümle, 0 ortak konu ve görünür SHA-256 içerik parmak izi ile veri
  ayrılığı doğrulanır; sonuç dış benchmark olarak sunulmaz.
- Modelsiz gerçek koşuda 70/80 doğru, %87,5 doğruluk, %90,18 Macro-F1 ve
  10 gizlenmeyen hata ölçülmüştür; model yüklü sonuç değişebilir.
- Üç teknik ölçüm ayrı SQLite anahtarlarında saklanır ve birbirini silmez.
- 184 yeni kontrol eklendi; toplam 50 pakette 1.164 otomatik test geçti.
- GitHub deposu ve `v1.4.0-teknofest-final` etiketi değiştirilmez.

## v1.4.1 — Bağlama Duyarlı Görüş Koruması

- Örtük erişim/süreklilik desteği, soru işaretsiz kanıt talebi ve uzaktan
  çalışmada ofis zorunluluğu bağlam üzerinden ayrıştırılır.
- Olumsuz denetim/kural/şart kökleri ve gerçek koşullu devam cümleleri
  doğru yorumlanır; hibrit ve modelsiz yedek yol tutarlı tutulur.
- 42 görülmemiş cümle ile 172 yeni regresyon testi eklendi.
- Toplam 46 test paketi ve 980 başarılı otomatik test doğrulandı.
- Teknik Doğrulama ekranında gerçek anlam koruması sayısı ve bağımsız
  tutma testi olmadığına ilişkin dürüst kalibrasyon notu gösterilir.
- Sabit `v1.4.0-teknofest-final` yarışma etiketi değiştirilmez.

## Önceki sürümlerin eksiksiz geçmişi
